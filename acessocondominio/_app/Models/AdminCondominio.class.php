...
class AdminCondominio {
    ...
    public function ExeDelete($UserId) {
        ...
        if (!$readUser->getResult()):
            ...
            $deleta = new Delete;
            $deleta->ExeDelete("unidades",  "WHERE id_condominio = :condo", "condo={$this->User}");

            $deleta = new Delete;
            $deleta->ExeDelete("proprietarios",  "WHERE condominios = :condo", "condo={$this->User}");

            $deleta = new Delete;
            $deleta->ExeDelete("cobranca",  "WHERE id_condominio = :condo", "condo={$this->User}");

            $deleta = new Delete;
            $deleta->ExeDelete("remessa",  "WHERE id_condominio = :condo", "condo={$this->User}");
    ...
    private function normalizeFloatFields() {
        $this->convertStringToFloat('multa');
        $this->convertStringToFloat('juros');
        $this->convertStringToFloat('taxa_fundo1');
        $this->convertStringToFloat('taxa_fundo2');
        $this->convertStringToFloat('taxa_fundo3');
        $this->convertStringToFloat('sindico_remu_valor');
        $this->convertStringToFloat('sindico_desc_valor');
        $this->convertStringToFloat('subsindico_desc_valor');
        $this->convertStringToFloat('subsindico_remu_valor');
        $this->convertStringToFloat('taxa_fundo4');
        $this->convertStringToFloat('taxa_fundo5');
        $this->convertStringToFloat('taxa');
    }

    private function convertStringToFloat($fieldName) {
        if($this->Data[$fieldName]):
            $this->Data[$fieldName] = $this->tofloat($this->Data[$fieldName]);
        endif;
    }
