...
class Check {
    ...
    /**
     * <b>Tranforma Data:</b> Transforma uma data no formato DD/MM/YY em uma data no formato DATE do Mysql!
   ...
   /**
    ...
    public static function Image($ImageUrl, $ImageDesc = null, $ImageW = null, $ImageH = null) {
        ...
        if (file_exists(self::$Data) && !is_dir(self::$Data)):
            ...
            return "<img style='max-width: 80px' src=\"{$base64}\" alt=\"{$ImageDesc}\" title=\"{$ImageDesc}\" style=\"height: {$ImageH}px\">";
        else:
            $imagem = HOME."/img/nopicture.jpg";
            return "<img style='max-width: 80px' src=\"{$imagem}\" alt=\"{$ImageDesc}\" title=\"{$ImageDesc}\">";
        endif;
    }
    public static function ImageUserAvatar($ImageUrl, $ImageDesc = null) {
        ...
        if (file_exists(self::$Data) && !is_dir(self::$Data)):
            ...
            $data = file_get_contents($imagem);
            $base64 = 'data:image/' . $type . ';base64,' . base64_encode($data);
            return "<img class=\"img-thumbnail\" src=\"{$base64}\" alt=\"{$ImageDesc}\" title=\"{$ImageDesc}\">";
        else:
            $imagem = HOME."/img/nopicture.jpg";
    ...
    public static function ImageUserAvatarMini($ImageUrl, $ImageDesc = null) {
        ...
        if (file_exists(self::$Data) && !is_dir(self::$Data)):
            ...
            $data = file_get_contents($imagem);
            $base64 = 'data:image/' . $type . ';base64,' . base64_encode($data);
            return "<img class=\"user-image\" src=\"{$base64}\" alt=\"{$ImageDesc}\" title=\"{$ImageDesc}\">";
        else:
            $imagem = HOME."/img/nopicture.jpg";
            return "<img class=\"user-image\" src=\"{$imagem}\" alt=\"{$ImageDesc}\" title=\"{$ImageDesc}\">";
        endif;
    }
}