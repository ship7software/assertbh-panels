 ...
 * Respnsável por gerenciar os Proprietarios no Admin do sistema!
 * @copyright (c) 2014, Marcus Correa
 */
class AdminProprietario {
    ...
    public function ExeDelete($UserId) {
        ...
        if (!$readUser->getResult()):
            $this->Error = ['Oppsss, você tentou remover um proprietario que não existe no sistema!', DS_ERROR];
            $this->Result = false;
        else:
    ...
    private function setData() {
        if (array_key_exists('condominios', $this->Data)):
            $condominios = $this->Data['condominios'];
            $this->Data['condominios'] = implode(", ", $condominios);
        else:
            $this->Data['condominios'] = '';
        endif;
    }
    ...
    private function Create() {
        ...
        if ($Create->getResult()):
            $this->Error = ["O proprietário <b>{$this->Data['nome']}</b> foi cadastrado com sucesso no sistema!", DS_ACCEPT];
    ...
    private function Update() {
        ...
        if ($Update->getResult()):
            $this->Error = ["O proprietário <b>{$this->Data['nome']}</b> foi atualizado com sucesso!", DS_ACCEPT];
            $this->Result = true;
        endif;
    }
    ...
    private function Delete() {
        ...
        if ($Delete->getResult()):
            $this->Error = ["Proprietário removido com sucesso do sistema!", DS_ACCEPT];
            $this->Result = true;
        endif;
    }
