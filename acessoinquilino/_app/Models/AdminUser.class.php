...
class AdminUser {
    ...
    public function ExeCreate(array $Data) {
        ...
        if ($this->Data['imagem']):
            $uplaod = new Upload;
            $uplaod->Image($this->Data['imagem'], null, null, 'usuarios/');
        endif;
    ...
    public function ExeUpdate($UserId, array $Data) {
        ...
        if (is_array($this->Data['imagem'])):
            ...
            $fotoUser = '../uploads/usuarios/' . $readImagem->getResult()[0]['imagem'];
            if (file_exists($fotoUser) && !is_dir($fotoUser)):
                unlink($fotoUser);
            endif;

            $uploadFoto = new Upload;
            $uploadFoto->Image($this->Data['imagem'], null, null, 'usuarios/');
        endif;
    ...
    public function ExeDelete($UserId) {
        ...
        if (!$readUser->getResult()):
            $this->Error = ['Oppsss, você tentou remover um usuário que não existe no sistema!', DS_ERROR];
            $this->Result = false;
        elseif ($this->User == $_SESSION['userlogininquilino']['id']):
            $this->Error = ['Oppsss, você tentou remover seu usuário. Essa ação não é permitida!!!', DS_INFOR];
            $this->Result = false;
        else:
            $DeleteDados = $readUser->getResult()[0];
            if (file_exists('../uploads/usuarios/' . $DeleteDados['imagem']) && !is_dir($DeleteDados['imagem'])):
                unlink('../uploads/usuarios/' . $DeleteDados['imagem']);
            endif;
            $this->Delete();
        endif;
    }
    ...
    public function getImagem($UserId) {
        ...
        $foto = '../uploads/usuarios/' . $readFoto->getResult()[0]['imagem'];
    ...
    private function Create() {
        ...
        if ($Create->getResult()):
            $this->Error = ["O usuário <b>{$this->Data['nome']}</b> foi cadastrado com sucesso no sistema!", DS_ACCEPT];
    ...
    private function Update() {
        ...
        if ($Update->getResult()):
            $this->Error = ["O usuário <b>{$this->Data['nome']}</b> foi atualizado com sucesso!", DS_ACCEPT];
            $this->Result = true;
        endif;
    }
    ...
    private function Delete() {
        ...
        if ($Delete->getResult()):
            $this->Error = ["Usuário removido com sucesso do sistema!", DS_ACCEPT];
            $this->Result = true;
        endif;
    }
